...
class ConvLSTM(nn.Module):
    def __init__(self, num_classes=4, hidden_dim=100, num_layers=2,
                 feature_col=7, resnet_output_dim=4, motion_f=False, motion_ratio_dim=0, image_f=True, resnet_fix_param=0):
        ...
        if resnet_fix_param > 0:
            for param in self.resnet.parameters():
                param.requires_grad = False

            # 最後から3つの層だけを更新対象に設定
            for layer in list(self.resnet.children())[-resnet_fix_param:]:
